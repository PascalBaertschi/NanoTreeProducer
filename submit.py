    ...
    for year in years:
      ...
      with open(samplelist, 'r') as file:
        for line in file:
          ...
          if args.type=='mc' and any(s in line[:len(s)+2] for s in ['SingleMuon','SingleElectron','EGamma','MET','SinglePhoton']): continue
          if args.type=='data' and not any(s in line[:len(s)+2] for s in ['SingleMuon','SingleElectron','EGamma','MET','SinglePhoton']): continue
      ...
      for channel in channels:
        ...
        for directory in directories:
            ...
            jobs         = open(jobList,'w')
            outdir       = ensureDirectory("/work/pbaertsc/heavy_resonance/output_%s/%s"%(year,name))
