...
from submit import args, bcolors, createJobs, getFileListLocal, getFileListPNFS, getFileListDAS, submitJobs, split_seq, saveFileListLocal
import itertools
import subprocess
...
def main():
    ...
    for year in years:
      ...
      # GET LIST
      samplelist = [ ]
      outdir     = "/work/pbaertsc/heavy_resonance/output_skim_%s/"%(year)
      for directory in sorted(os.listdir(outdir)):
          ...
          if args.type=='mc' and any(s in directory[:len(s)+2] for s in ['SingleMuon','SingleElectron','EGamma','MET']): continue
          if args.type=='data' and not any(s in directory[:len(s)+2] for s in ['SingleMuon','SingleElectron','EGamma','MET']): continue
      ...
      for channel in channels:
        ...
        for directory in samplelist:
            ...
            outdir       = "/work/pbaertsc/heavy_resonance/output_skim_%s/%s"%(year,directory)
            ...
            jobList = '/work/pbaertsc/heavy_resonance/NanoTreeProducer/joblist/joblist_%s_%s%s_retry.txt'%(directory,channel,tag)
