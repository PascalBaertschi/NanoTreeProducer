...
class AK8btagProducer(Module):
    ...
    def analyze(self, event):
        ...
        if not self.isData:
            self.btagTool.fillEfficiencies(event,idx_fatjet)
            print self.btagTool.hists['c']
            self.outfile.btag_eff_c = self.btagTool.hists['c']
            self.outfile.btag_eff_b = self.btagTool.hists['b']
            self.outfile.btag_eff_udsg = self.btagTool.hists['udsg']
            self.outfile.btag_eff_c_all = self.btagTool.hists['c_all']
            self.outfile.btag_eff_b_all = self.btagTool.hists['b_all']
            self.outfile.btag_eff_udsg_all = self.btagTool.hists['udsg_all']
            return True
        else:
            return False
