...
def main():
    ...
    for year in years:
      ...
      for directory in sorted(os.listdir(outdir)):
          ...
          if args.type=='mc' and any(s in directory[:len(s)+2] for s in ['SingleMuon','SingleElectron','EGamma','MET','SinglePhoton']): continue
          if args.type=='data' and not any(s in directory[:len(s)+2] for s in ['SingleMuon','SingleElectron','EGamma','MET','SinglePhoton']): continue
      ...
      for channel in channels:
        ...
        for directory in samplelist:
            ...
            outdir       = "/work/pbaertsc/heavy_resonance/output_%s/%s"%(year,directory)
