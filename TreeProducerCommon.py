...
def getJetID(year,event,idx):
  eta = abs(event.Jet_eta[idx])
  neutral_hadron_frac = event.Jet_neHEF[idx]
  neutral_em_frac = event.Jet_neEmEF[idx]
  number_constituents = event.Jet_nConstituents[idx]
  charged_hadron_frac = event.Jet_chHEF[idx]
  charged_em_frac = event.Jet_chEmEF[idx]
  if year == 2016:
    ...
    if 3.0 < eta and eta <= 5.0:
      if neutral_hadron_frac > 0.02 and neutral_em_frac < 0.9:
        return True
      else:
        return False
    elif 2.7 < eta and eta <= 3.0:
